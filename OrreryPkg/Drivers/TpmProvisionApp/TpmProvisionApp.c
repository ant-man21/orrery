  ...
  )
{
  ...
  Print (L"[PROVISION] %u total EFI_FIRMWARE_VOLUME2_PROTOCOL handle(s)\n", (UINT32)NumHandles);
  ...
  for (Index = 0; Index < NumHandles; Index++) {
    ...
    if (EFI_ERROR (Status)) {
      Print (L"[PROVISION]   handle %u: no FVB2 protocol — skipped\n", (UINT32)Index);
      continue;
    }

    Status = Fvb->GetPhysicalAddress (Fvb, &FvAddress);
    if (EFI_ERROR (Status)) {
      Print (L"[PROVISION]   handle %u: FVB2 GetPhysicalAddress failed: %r — skipped\n", (UINT32)Index, Status);
      continue;
    }
    ...
    Print (
      L"[PROVISION]   handle %u: addr=0x%lx size=0x%lx guid=%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x gcdType=%d%a%a\n",
      (UINT32)Index,
      FvAddress,
      FvHeader->FvLength,
