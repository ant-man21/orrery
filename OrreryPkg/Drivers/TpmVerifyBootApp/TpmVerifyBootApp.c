  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    Print (L"[VERIFY] GetPlatformRomInfo failed: %r\n", Status);
    return Status;
  }
   ...
   */
  Print (L"[VERIFY] PlatformRomInfoLib: base=0x%lx size=0x%lx (%lu KB)\n", RomBase, RomSize64, RomSize64 / 1024);

  if ((RomBase == 0) || (RomSize64 == 0)) {
    Print (L"[VERIFY] PlatformRomInfoLib returned an empty ROM region — refusing to read\n");
    return EFI_NOT_FOUND;
  }

  if (RomSize64 > SIZE_64MB) {
    Print (L"[VERIFY] PlatformRomInfoLib returned an implausibly large size — refusing to read\n");
  ...
  *RomBuffer = AllocateCopyPool (*RomSize, (VOID *)(UINTN)RomBase);
  ...
  Print (L"[VERIFY] ROM snapshot: 0x%lx, %u bytes\n", RomBase, (UINT32)*RomSize);
  return EFI_SUCCESS;
}
