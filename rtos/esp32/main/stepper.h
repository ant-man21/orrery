...
#define REALTIME_TICK_INTERVAL_US 400000u /* phase 1: fixed 0.4 second per degree for every motor,
                                              until real per-planet rates arrive over the network */
...
typedef enum {
    ...
    MODE_REALTIME   /* tick-based ("watch tick"), driven by (eventually) real ephemeris data */
} SystemMode;
...
/* Must be called once (from app_main, before any task uses SPI) with a
 * handle already attached to the shift-register chain via
 * spi_bus_add_device() on MOTOR_SPI_HOST. */
