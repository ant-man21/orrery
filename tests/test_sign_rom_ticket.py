...
#
# "approved" and "a_hash" are specific to PCR_FOR_BIOS -- this project uses
# PCR15 (non-resettability, issue #27), so they were computed by hand against
# pcr_selection_bytes(15), not by importing sign_rom_ticket.py itself. The
# earlier three layers (inner/pcr_ext/pcrdig) never touch which PCR is
# selected, so they're unaffected by that choice.
GOLDEN_ROM = b"orrery-unit-test-rom"
GOLDEN = {
    "inner":    "145554d2ad093590f4c0a74d938f7ccb4d952c83ef943d055e0134e8df1b08db",
    "pcr_ext":  "328218c9d7bfac7bf3d50dc5bd28c90db566f7a2e5b6934b65a42bbf7e2b901d",
    "pcrdig":   "e7a4a541c0ca9a15769679b3d0455e0afb49b64fd0067497cf6ae38e076b1d7b",
    "approved": "a0dfde3f59498379bf49cf41ba263cf57acc0fbb5e657f2ce57f38e3b4082b7e",
    "a_hash":   "d83b2a449bb0a7cfcdb0b40b4b3a8dfd79ce7efbdf58b1908f55b965378708e0",
}
...
def test_pcr_selection_bytes_for_pcr15(sign_rom_ticket):
    """PCR15 is PCR_FOR_BIOS -- the one this project actually uses."""
    got = sign_rom_ticket.pcr_selection_bytes(15)
    assert got == bytes.fromhex("00000001000b03008000")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (15, "008000"),   # byte 1, bit 7  — PCR_FOR_BIOS, what this project uses
...
def test_compute_a_hash_matches_golden_vectors(sign_rom_ticket):
    inner, pcr_ext, pcrdig, approved, a_hash = sign_rom_ticket.compute_a_hash(GOLDEN_ROM)
    assert inner.hex()    == GOLDEN["inner"]
    assert pcr_ext.hex()  == GOLDEN["pcr_ext"]
    assert pcrdig.hex()   == GOLDEN["pcrdig"]
    assert approved.hex() == GOLDEN["approved"]
    assert a_hash.hex()   == GOLDEN["a_hash"]


def test_pcr_ext_layer_is_a_real_pcr_extend(sign_rom_ticket):
    """pcr_ext must be extend(0x00*32, inner), i.e. what one TPM2_PCR_Extend
    against a freshly-reset PCR produces — not SHA256(rom) directly."""
    inner, pcr_ext, *_ = sign_rom_ticket.compute_a_hash(GOLDEN_ROM)
    assert pcr_ext == hashlib.sha256(b"\x00" * 32 + inner).digest()
    assert pcr_ext != inner
